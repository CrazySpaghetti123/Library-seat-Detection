# seat-detection Specification

## Purpose
以 YOLOv8 + ROI 影像偵測判定每個座位的實際使用狀態（有人／空位／物品佔位），經去抖動後將狀態變化同步至後端。
## Requirements
...
系統 SHALL 辨識「人已離開但物品（背包、筆電、書本、水壺等）仍在 ROI 內」的情形，並將其標記為疑似佔位事件，作為離席逾時計時（seat-timeout）的觸發來源。

#### Scenario: Person leaves but belongings remain (人離開、物品仍在)

- **WHEN** 座位 A1 為 `OCCUPIED`，YOLOv8 連續 5 秒以上未在 ROI 內偵測到「人」，但仍偵測到「背包／筆電／書本／水壺」等物品
